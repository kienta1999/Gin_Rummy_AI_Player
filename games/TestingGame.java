...
/**
 * This class is a copy of GinRummyGame, except that anytime a player takes an 
 * erroneous action (e.g. attempting to discard a card it does not hold),
 * rather than forfeiting, a BadActionException is thrown, causing the code to crash immediately. 
 * This makes it easier to identify when our players are taking erroneous actions.
 * Extra debugging information is also displayed.
 * 
 * @author Todd W. Neller, Steven Bogaerts
 * @version 1.0

Copyright (C) 2021 Todd Neller
...
public class TestingGame {
	...
	public static void setPlayVerbose(boolean playVerbose) {
		TestingGame.playVerbose = playVerbose;
	}
	 ...
	 */
	public TestingGame(GinRummyPlayer player0, GinRummyPlayer player1) {
		...
		while (scores[0] < GinRummyUtil.GOAL_SCORE && scores[1] < GinRummyUtil.GOAL_SCORE) { // while game not over
			...
			if (knockMelds != null) { // round didn't end due to non-knocking and 2 cards remaining in draw pile
				...
				if (knockingDeadwood > GinRummyUtil.MAX_DEADWOOD) {
					...
					reportGameInfo(hands, currentPlayer, discards);
					throw new BadActionException("Player " + currentPlayer + " melds " + knockMelds + " with greater than " + knockingDeadwood + " deadwood and forfeits.");
					//return opponent;
				}
				...
				for (ArrayList<Card> meld : opponentMelds) {
					...
					if (!GinRummyUtil.getAllMeldBitstrings().contains(meldBitstring) // non-meld ...
						...
						reportGameInfo(hands, currentPlayer, discards);
						throw new BadActionException("Player " + opponent + " melds " + opponentMelds + " illegally and forfeits.");
	...
	/**
	 * Test and demonstrate the use of the TestingGame class.
	 * @param args (unused)
	 */
	public static void main(String[] args) {
		// Single verbose demonstration game
		setPlayVerbose(true);
		new TestingGame(new SimpleGinRummyPlayer(), new SimpleGinRummyPlayer()).play();
		...
		// TestingGame game = new TestingGame(new SimpleGinRummyPlayer(), new SimpleGinRummyPlayer());
		EnsembleHandEvalPlayer knockOnGinPlayer = new EnsembleHandEvalPlayer(new ParamList(new double[]{1}), new DeadwoodHandEvaluator());
        knockOnGinPlayer.setKnockDecider(new KnockOnGinKnockDecider());
        knockOnGinPlayer.setDrawDecider(new DeadwoodDrawDecider());

		TestingGame game = new TestingGame(new SimpleGinRummyPlayer(), knockOnGinPlayer);
